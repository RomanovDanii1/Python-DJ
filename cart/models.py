...
class Order(models.Model):
    ...
    def get_cart(user: User):
        ...
        if not cart:
            cart = Order.objects.create(user=user, status=Order.STATUS_CART, amount=Decimal(0))
        return cart
...
class OrderItem(models.Model):
    ...
    product = models.ForeignKey(Pet, on_delete=models.PROTECT)
